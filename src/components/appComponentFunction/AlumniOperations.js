...
const searchAlumni = (originalAlumni, setAlumni, search) => {
...
// for filtering alumni by department
const filterAlumni = (originalAlumni, setAlumni, filter) => {
  setAlumni(originalAlumni);
  setAlumni((alumniList) => {
    return alumniList.filter((alumnus) => {
      const mergedName = alumnus.department.toLowerCase();
      return mergedName.includes(filter.toLowerCase());
    });
  });
};
